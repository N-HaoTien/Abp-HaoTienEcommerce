...
namespace HaoTienEcommerce.Products
{
    public class ProductCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(HaoTienEcommerceConsts.ProductIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(HaoTienEcommerceConsts.ProductIdentitySettingId, "Sản phẩm", HaoTienEcommerceConsts.ProductIdentitySettingPrefix, 1, 1));
